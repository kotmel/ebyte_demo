...
#include "radio_config_850_1b.h"

/* 辅助识别模块频段 */
#if defined(EBYTE_E30_900M20S)
static uint8e_t E30x_NameString[] = "E30-900M20S";
#endif

/* 辅助识别驱动程序版本号 */
#define EBYTE_E30_PROGRAM_TYPE 0x10
...
///工作模式切换
#define  E30X_NOCHANGE            0x00
#define  E30X_SLEEP               0x01
#define  E30X_SPI_ACTIVE          0x02
#define  E30X_READY               0x03
#define  E30X_TX_TUNE             0x05
#define  E30X_RX_TUNE             0x06
#define  E30X_TX                  0x07
#define  E30X_RX                  0x08

uint8e_t E30x_GetIRQ( void );
...
/// 状态标识
static E30x_Status_t E30x_Status = GO_INIT;

static uint8e_t E30x_RxBuffer[64];

///WDS生成的配置文件
const uint8e_t E30x_Config_WDS[] = RADIO_CONFIGURATION_DATA_ARRAY;
 ...
 *                  请检查SPI软件通信配置是否正确
 */
static uint8e_t E30x_IsReady( void )
{
    uint8e_t cts;
    uint8e_t result = 0;
 ...
 * @return 0:正常 1:超时
 */
static uint8e_t E30x_SendCommand( uint8e_t* data, uint8e_t size )
{
    uint8e_t result = 0;
    /* 检查:模块是否准备完成 */
    result = E30x_IsReady();
 ...
 * @return 0:正常 1:超时
 */
static uint8e_t E30x_GetResponse( uint8e_t* data, uint8e_t size )
{
    uint8e_t result = 0;
    /* 检查:模块是否准备完成 */
    result = E30x_IsReady();
 ...
 * @return 0:正常 1:超时
 */
static uint8e_t E30x_SetProperty( uint16e_t address, uint8e_t data )
{
    uint8e_t result = 0;
    uint8e_t cmd[5];
 ...
 */
static uint8e_t E30x_SetWorkMode( uint8e_t state )
{
    uint8e_t cmd[3];
    uint8e_t i;
    uint8e_t result = 0;
    ...
    for( i = 0 ; i < 5; i++ )
    {
        ...
        cmd[0] = 0x33;
        /* 写入 */
        E30x_SendCommand( cmd, 1 );
   
        /* 读取指令响应 */
        E30x_GetResponse( cmd, 3 );
 ...
 * @return 0:正常 1:超时
 */
static uint8e_t E30x_ClearFIFO( uint8e_t mode )
{
    uint8e_t cmd[2];
    uint8e_t result;
    if( mode == FIFO_WRITE )
 ...
 */
static void E30x_SetFIFO( uint8e_t* data, uint8e_t size )
{
    /* 检查:模块是否准备完成 */
    E30x_IsReady();
    /* SPI CS */
 ...
 */
static uint8e_t E30x_GetFIFO( uint8e_t* data, uint8e_t* size )
{
    uint8e_t length;
    uint8e_t result ;
    /* 检查模块是否准备完成 */
...
static uint8e_t ttio[10];
uint8e_t E30x_SetGPIO(void)
{
    uint8e_t cmd[10];
    /* 指令:GPIO_PIN_CFG(0x13) 引脚配置 */
    cmd[0] = 0x13;
    cmd[1] = 35;
    cmd[2] = 24;
    cmd[3] = 33|0x40;
    ...
    cmd[6] = 0;
    cmd[7] = 0;
    E30x_SendCommand(cmd, 8);
    E30x_GetResponse(ttio, 8);    
  return 0;
}
...
uint8e_t E30x_GetIRQ( void )
{
    ...
    /* 写入 */
    E30x_SendCommand( cmd, 4 );
    /* 读回指令响应 */
    E30x_GetResponse( buffer, 9 );
 ...
 * @return 0:正常 1:异常
 */
static uint8e_t E30x_IsExist( void )
{
    uint8e_t cmd[4];
    ...
    /* 读回指令响应 */
    E30x_GetResponse( buffer, 9 ); 
    /* 正常情况下 读回数据中的PART信息是0x4463 */
    if( (buffer[2] == 0x44) && (buffer[3] == 0x63) ) return 0;//正常返回
 ...
 */
static void E30x_Config( void )
{
    uint8e_t i;
    uint16e_t j = 0;
    /* 链接WDS自动生成的参数表  */
    const uint8e_t*  ptr = E30x_Config_WDS;
    /* 写入WDS参数表 */
    while( ( i = *( ptr + j ) ) != 0 )
    {
        j += 1;
        E30x_SendCommand( ( uint8e_t* )ptr + j, i );
        j += i;
    }
     
    /* 配置属性 可变长数据包 */
    E30x_SetProperty( 0x1208, 0x2A ); //大端模式 1Bbyte长度且值存放在FIFO中 第2区域数据包长度可变
    E30x_SetProperty( 0x1209, 0x01 ); //数据包长度放在第1区
    E30x_SetProperty( 0x120D, 0x00 ); //第一区域长度 高字节
    E30x_SetProperty( 0x120E, 0x01 ); //第一区域长度 低字节 0x01:限制为一个字节
    E30x_SetProperty( 0x1211, 0x00 );
    E30x_SetProperty( 0x1212, 0x10 );
    /* 配置输出功率
     * 输入范围:0x00至0x7F   默认值:0x7F
     * 注意;功率并不是线性增加
     * 当接近最大值时，步进可能小于0.1db
     * 当功率偏低时，步进可能大于0.1db  */
    E30x_SetProperty( 0x2201, 0x7F );
...
uint8e_t E30x_GoReceive( void )
{
    ...
    uint8e_t cmd[8];
    /* FIFO 复位 */
    result = E30x_ClearFIFO( FIFO_READ );
    if( result != 0 )
    {
        return 1;
    }
    E30x_ClearFIFO( FIFO_WRITE );
    /* 切换模式:  SPI_ACTIVE
     * 请参考  datasheet 中的工作模式切换流程图
     * 接收模式流程为: Sleep或Stby->SPI active->Ready->RxTune->Rx */
    result = E30x_SetWorkMode( E30X_SPI_ACTIVE );
    ...
    /* 切换模式:  RX_TUNE  接收预备状态 */
    result = E30x_SetWorkMode( E30X_RX_TUNE );
    if( result != 0 )
    {
        return 1;
    }
    ...
    cmd[3] = 0;         //接收长度:   高Byte
    cmd[4] = 0;         //接收长度:   低Byte
    cmd[5] = E30X_RX;   //接收超时后: 切换为接收状态
    cmd[6] = E30X_RX;   //接收成功后: 切换为接收状态
    cmd[7] = E30X_RX;   //接收失败后: 切换为接收状态
    E30x_SendCommand( cmd, 8 );
    
    /* 状态记录 */
    E30x_Status = GO_WAIT_RECEIVE;
...
uint8e_t E30x_GoTransmit( uint8e_t* data, uint8e_t size )
{
    ...
    /* FIFO 写入待发送数据 */
    E30x_SetFIFO( data, size );
    /* 模式切换: E30X_TX_TUNE 发送预备模式 */
    result = E30x_SetWorkMode( E30X_TX_TUNE );
    if( result != 0 )
    {
        return 1;
    }
    ...
    cmd[5] = 128;          //发送延时128us
    E30x_SendCommand( cmd, 6 );
    
    /* 状态记录 */
    E30x_Status = GO_WAIT_TRANSMIT;
    
    /* 阻塞发送方式 在此等待发送完成 */
    uint8e_t irqStatus = 0;
    do
    {
        irqStatus = E30x_GetIRQ();
    }
...
{
    uint8e_t result = E30x_SetWorkMode( E30X_SLEEP );
    if( result != 0 )
    {
        return 1;
    }
...
uint8e_t E30x_TaskForPoll( void )
{
    ...
    if( irqStatus & 0x10 )
    {
        uint8e_t recvSize = 0;
        /* 获取数据 */
        E30x_GetFIFO( E30x_RxBuffer, &recvSize );
        
        /* 状态记录 */
        E30x_Status = GO_STBY;
        
        /* 回调用户函数 */
        Ebyte_Port_ReceiveCallback( 0x0002, E30x_RxBuffer, recvSize );
    }
    return 0;
}
